...
# --- FUNCIONES NÚCLEO (Extraídas de tu script original) ---
#
...
with tabs[0]:
    ...
    uploaded_d = st.file_uploader("Subir archivo RINEX comprimido", type=["24d", "25d", "26d", "d"])
    
    if uploaded_d:
        # Sanitización de nombre según tu script
...
with tabs[1]:
    ...
    with c2: up_obs = st.file_uploader("Archivo de Observación .O", type=["o", "obs", "24o", "25o"])
    
    if up_pos and up_obs:
        if st.button("🔗 Vincular Coordenadas"):
            # Lógica de lectura de .pos
...
with tabs[2]:
    ...
    up_rnx = st.file_uploader("Subir RINEX con APPROX POSITION XYZ", type=["o", "obs", "24o", "25o"])
    ...
    if up_rnx:
        if st.button("📐 Calcular PNEZD"):
            ...
            if match:
                ...
                trans = pyproj.Transformer.from_proj(ecef, lla, always_xy=True)
                ...
                proj = pyproj.Transformer.from_crs("EPSG:4326", f"EPSG:{faja}", always_xy=True)
